...
export const config = {
  ...
  contextMessages: parseInt(process.env.THINKER_CONTEXT_MESSAGES || "6", 10),
  maxTokens: parseInt(process.env.THINKER_MAX_TOKENS || "1024", 10),
  completionTokens: parseInt(process.env.THINKER_COMPLETION_TOKENS || "120", 10),
  maxReplyChars: parseInt(process.env.THINKER_MAX_REPLY_CHARS || "400", 10),
  maxContextLineChars: parseInt(process.env.THINKER_MAX_CONTEXT_LINE_CHARS || "320", 10),
  maxRetries: parseInt(process.env.THINKER_MAX_RETRIES || "4", 10),
  minReplyChars: parseInt(process.env.THINKER_MIN_REPLY_CHARS || "10", 10),
  mechanicalPrompt: process.env.THINKER_MECHANICAL_PROMPT === "true",
  // Cold start needs a non-empty seed — blank messages 400 on Claude / become assistant-mode.
  seedOpener: process.env.THINKER_SEED_OPENER !== "false",
